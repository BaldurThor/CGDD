...
func init(weapon_type: WeaponType, player_stats: PlayerStats) -> void:
	_weapon_type = weapon_type
	_player_stats = player_stats

func _ready() -> void:
	_on_player_stats_range_changed()
	_player_stats.range_changed.connect(_on_player_stats_range_changed)
	target_range_shape.debug_color = _weapon_type.attack_range_debug_color
...
func _on_player_stats_range_changed() -> void:
	target_range_shape.shape.radius = _weapon_type.range * _player_stats.ranged_accuracy_mod

## Retrieves a single enemy based on the weapon type's target priority.
func get_target(target_priority: consts.TargetPriority) -> Enemy:
...
func _order_targets_by_strength(targets: Array[Enemy]) -> void:
	targets.sort_custom(func(a, b): a.entity_stats.get_health_percentage() < b.entity_stats.get_health_percentage())
